...
const cli = spawnSync(process.execPath, ["scripts/release-readiness.mjs", "wrangler.test-missing.jsonc"], {
  cwd: new URL("../", import.meta.url),
  encoding: "utf8",
});
assert.equal(cli.status, 2);
assert.match(cli.stdout, /"status": "blocked"/);
assert.match(cli.stdout, /owner_approval_required:wrangler\.test-missing\.jsonc/);

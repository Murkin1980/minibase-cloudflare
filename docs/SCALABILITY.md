...
| **F. Transactions** | Control plane: `batch()` is atomic. Data plane: one statement per REST call | `create order + tasks + event` cannot be atomic today. The D1 REST API does not expose multi-statement transactions | Server-side commands; multi-row single-statement inserts are atomic in SQLite and work over the REST API | P1 (CP-05) |

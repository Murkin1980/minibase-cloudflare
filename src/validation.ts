...
import { publishableKeyScopes, secretKeyScopes } from "./key-scopes";
...
export function parseCreateDataKey(value: unknown): CreateDataKeyRequest {
  if (!value || typeof value !== "object") throw new Error("body_must_be_object");
  const candidate = value as Record<string, unknown>;
  if (typeof candidate.name !== "string" || candidate.name.trim().length < 2 || candidate.name.length > 80) {
    throw new Error("invalid_name");
  ...
  const allowed = candidate.kind === "publishable"
    ? new Set<string>(publishableKeyScopes)
    : new Set<string>(secretKeyScopes);
